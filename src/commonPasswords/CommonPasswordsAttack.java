...
public class CommonPasswordsAttack extends AttackAPI {
	...
	private int currentLevenschteinDistance = 10;
	private String levenGuess = "";
	
	public static void main(String[] args) {
		AttackAPI cpa = new CommonPasswordsAttack("awefweaf23aqf32awf");
	...
	private int calculateLevenschtein(String s1, String s2) {
		if (s1 == null) {
			...
			return levenschtein(s1, s2, 0, 0, 0);
		}
	}
	
	// TODO ANTHONY Consider memoization if too slow
	private int levenschtein(String s1, String s2, int length1, int length2, int currentDistance) {
		if (currentDistance > currentLevenschteinDistance) {
			return Integer.MAX_VALUE;
		}
		else if (s1.length() == length1 || s2.length() == length2) {
			int temp = Math.max(s1.length() - length1, s2.length() - length2) + currentDistance;
			return temp > currentLevenschteinDistance? Integer.MAX_VALUE : temp;
		} else {
			int lev1 = (s1.charAt(length1) == s2.charAt(length2)) ? levenschtein(s1, s2, length1 + 1, length2 + 1, currentDistance) : levenschtein(s1, s2, length1 + 1, length2 + 1, currentDistance + 1);
			int lev2 = levenschtein(s1, s2, length1 + 1, length2, currentDistance + 1);
			int lev3 = levenschtein(s1, s2, length1, length2 + 1, currentDistance + 1);
